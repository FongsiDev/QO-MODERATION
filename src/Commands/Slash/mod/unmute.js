module.exports = new global.SlashCommandBuilder()
  ...
  .setRun(async (interaction, client, data) => {
    ...
    if (!member.roles.cache.has(muterole.id))
      return client.send("**User Is Not Muted!**", {
        ephemeral: true,
        interaction,
      });
    let roles = client.db.get(
      `memberid_${interaction.guild.id}_${member.id}`,
      userRoles
    );
    try {
      const sembed2 = new interactionEmbed()
        .setColor("RED")
        .setDescription(
          `**You Have Been unmuted From ${interaction.guild.name} for ${reason} Unmuted by ${interaction.user.username}**`
        )
        .setFooter(interaction.guild.name, interaction.guild.iconURL());
      member
        .send({ embeds: [sembed2] })
        .then(async () => {
          await member.roles.set(roles).then(async () => {
            client.db.delete(`memberid_${interaction.guild.id}_${member.id}`);
            if (data.modLogs) {
            ...
            return client.send(
              `${member} has been Unmuted! Reason: **${reason}**`,
              {
                interaction,
              }
            );
